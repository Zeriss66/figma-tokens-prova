...
class Search extends LitElement {
      ...
      <form method="dialog" role="search" novalidate="">
        <label aria-label="Search">
          <input
            ...
            @input=${(e) => this.updateHits(e.target.value)}
            @focus=${(e) => this.updateHits(e.target.value)}
          />
        </label>
      </form>
      <ul class="hits">
        ${this.hits === null ? html`<li><i>Type for searching...</i></li>` : this.hits.length === 0 ? html`<li><i>No match found</i></li>` : this.hits.map((hit) => html`<li>
  ...
  async updateHits(searchValue) {
    var _a;
    if (!searchValue) {
      this.hits = null;
    } else {
      this.hits = (await ((_a = this.search) == null ? void 0 : _a.call(this, searchValue))).filter(Boolean).slice(0, 10).map(this.mark).map((hit) => {
  ...
  mark(hit) {
    for (const t of hit.terms.sort((a, b) => b.length - a.length))
      for (const m of hit.match[t])
        hit[m] = hit[m].replace(new RegExp(`([^\0]?)(${t})([^\0]?)`, "gi"), "$1\0$2\0$3").replace(/</gm, "&lt;").replace(/>/gm, "&gt;");
    return hit;
  }
  highlight(content) {
    ...
    if (first >= 0) {
      ...
      const end = Math.min(Math.max(last, first) + afterHitMatch, content.length);
      content = (start > 0 ? "..." : "") + content.slice(start, end) + (end < content.length - 1 ? "..." : "");
      return content.replace(/\0(.*?)\0/g, '<strong class="highlight">$1</strong>');
    }
  }
  hideHits() {
